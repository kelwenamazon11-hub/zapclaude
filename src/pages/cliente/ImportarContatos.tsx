...
export default function ImportarContatos() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const allowed = files.filter((f) =>
      [".txt", ".csv", ".xlsx", ".xls", ".pdf"].some((ext) => f.name.toLowerCase().endsWith(ext))
    );
    setArquivos((prev) => [...prev, ...allowed]);
    if (allowed.length < files.length) {
      toast({ title: "Arquivo ignorado", description: "Apenas TXT, CSV, Excel e PDF são aceitos.", variant: "destructive" });
    }
  };
  ...
  const handleImportar = () => {
    setImportando(true);
    setTimeout(() => {
      const numeros = activeTab === "texto" ? textoNumeros.split(/[\n,;]+/).filter(Boolean).length : Math.floor(Math.random() * 500) + 50;
      const invalidos = Math.floor(numeros * 0.08);
      setResultado({ validos: numeros - invalidos, invalidos });
      setImportando(false);
      toast({ title: "Importação concluída!", description: `${numeros - invalidos} contatos válidos importados.` });
    }, 2000);
  };
    ...
    <AppLayout>
      <div className="max-w-3xl mx-auto space-y-8">
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p className="text-muted-foreground mt-1">Adicione números via texto, Excel ou PDF</p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-6 space-y-6">
          {activeTab === "texto" ? (
            ...
            <div className="space-y-4">
              ...
              >
                ...
                <p className="text-xs text-muted-foreground mt-1">TXT, CSV, Excel, PDF</p>
              </div>
              <input ref={fileRef} type="file" multiple accept=".txt,.csv,.xlsx,.xls,.pdf" className="hidden" onChange={handleFileChange} />
          ...
          {resultado && (
            <div className="flex gap-4 p-4 rounded-lg bg-secondary/50 border border-border">
              <div className="flex items-center gap-2 text-green-400">
                ...
                <span className="text-sm font-medium">{resultado.validos} válidos</span>
              </div>
              <div className="flex items-center gap-2 text-red-400">
                <AlertCircle className="w-4 h-4" />
                <span className="text-sm font-medium">{resultado.invalidos} inválidos</span>
              </div>
            </div>
          )}
          ...
          <Button
            ...
            disabled={importando || (activeTab === "texto" ? !textoNumeros.trim() : arquivos.length === 0)}
